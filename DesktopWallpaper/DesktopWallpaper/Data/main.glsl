...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Output to screen
    fragColor = vec4(col, 1.0) + texture(iChannel0, fragCoord.xy / iResolution.xy);
}

// Function for pure glsl code
void main(){vec4 color=vec4(0.0,0.,0.,1.);mainImage(color,gl_FragCoord.xy);color.rgb=clamp(color.rgb,0.,1.);color.w=1.0;gl_FragColor=color;}